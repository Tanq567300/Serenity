...
const MoodEntry = ({ onSubmit, todaysMood, isSubmitting }) => {
  ...
  return (
    <div className="bg-white rounded-lg lg:rounded-xl shadow-sm border border-gray-200 p-4 lg:p-6" style={{marginBottom: '20px', paddingBottom: '20px', paddingTop: '20px', paddingLeft: '20px', paddingRight: '20px'}}>
      ...
      <div className="mb-4 lg:mb-6">
        <div className="grid grid-cols-5 gap-2 lg:gap-3">
          {moodOptions.map((option) => (
            <button
              key={option.value}
              className={`flex flex-col items-center p-2 lg:p-4 rounded-lg border-2 transition-all duration-200 ${
            ...
            >
              <div className="text-lg lg:text-2xl mb-1 lg:mb-2">{option.emoji}</div>
              <div className="text-xs lg:text-sm font-medium truncate">{option.label}</div>
      ...
      <div className="text-center mb-4 lg:mb-6 p-3 lg:p-4 bg-gray-50 rounded-lg">
        <div className="text-3xl lg:text-4xl mb-2">{selectedMoodOption.emoji}</div>
      ...
      <button
        ...
        className={`w-full flex items-center justify-center px-4 py-3 rounded-lg text-white font-medium transition-all duration-200 text-sm lg:text-base ${
          isSubmitting
      ...
      >
        {isSubmitting ? (
          <>
            <div className="w-4 h-4 lg:w-5 lg:h-5 animate-spin rounded-full border-2 border-white border-t-transparent mr-2"></div>
            Saving...
          </>
        ) : (
          <>
            <SimpleIcons.Check className="w-4 h-4 lg:w-5 lg:h-5 mr-2" />
      ...
      {todaysMood && !isSubmitting && (
        <div className="mt-4 p-3 lg:p-4 bg-blue-50 rounded-lg">
          <div className="flex items-center space-x-2 lg:space-x-3">
            <div className="text-xl lg:text-2xl">{moodOptions.find(o => o.value === todaysMood.mood)?.emoji}</div>
            <div className="min-w-0 flex-1">
              <div className="font-medium text-blue-900 text-sm lg:text-base">
                Today's mood: {moodOptions.find(o => o.value === todaysMood.mood)?.label}
              </div>
              {todaysMood.note && (
                <div className="text-xs lg:text-sm text-blue-700 truncate">"{todaysMood.note}"</div>
              )}
            </div>
          </div>
        </div>
